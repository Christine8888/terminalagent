...
def trim_conversation(conversation: List[BetaMessageParam], keep_imgs: int = 1) -> List[BetaMessageParam]:
    image_positions = []

    # Find all images, including ones nested in tool_result content
                ...
                if isinstance(content.get('content'), list):
                    for nested_content in content['content']:
                        if nested_content.get('type') == "image":
                            image_positions.append((msg_idx, content_idx))
                            break  # Only need to mark this tool_result once if it has images
    ...
    for msg_idx, msg in enumerate(conversation):
        ...
        for content_idx, content in enumerate(msg['content']):
            ...
            if content['type'] in ["text", "tool_use"]:
                # Keep all text and tool_use content
                ...
                if position in images_to_keep:
                    # Keep tool_result if it contains a recent image
                    new_content.append(content)
                else:
                    # For tool_results not in images_to_keep, keep them but remove their images
                    if isinstance(content.get('content'), list):
                        filtered_content = [
...
def main():
    instructions = (
        """
        Ask the user any clarifying questions when needed. 
        You are in Row 50 of the FX Reimbursements 24-25 Google Sheet.
        Only record values from a box that you have selected. Do not look at any other rows. Do not use Alt Tab.

        START FROM STEP 11. Check every step before proceeding. If a step fails, try to fix it and do not move on.
        ...
        2. Find column E ("Project Name"). Remember this as the project name
        ...
        8. Switch back to the FX Reimbursements 24-25 Google Sheet tab
        9. Go one column to the right, so that you are in Column G. Click on the link. This will open a pop-up.
        10. Click on the image in the pop-up. This will open a new tab.
        10.5 Wait for the document to load.
        11. Download the document using the keystrokes ctrl+s. 
        11.5 Wait for the document to download.
        12. Switch back to the FX Reimbursements 24-25 Tab.
        13. Go one column to the right, so that you are in Column H. Click on the link. This will open a pop-up.
        14. Click on the image in the pop-up. This will open a new tab.
        14.5 Wait for the document to load.
        15. Download the document using the keystrokes ctrl+s.
        15.5 Wait for the document to download.
        16. Switch back to the FX Reimbursements 24-25 Tab.
        17. Move 4 more columns to the right, so that you are in Column K. Make note of the Vendor name, Total amount and Date of purchase.
        13. Navigate back to the Payment Details tab.
